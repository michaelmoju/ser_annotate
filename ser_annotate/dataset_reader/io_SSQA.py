...
from ..std import *
from ser_annotate.dataset_reader.Const import sep_bar1, sep_bar2
...
def str_lesson2st(lesson_str):
	'''
	output: List[paragraph]
	where paragraph is [start, end, List[Sentence]]
	where sentence is [start, end, sentence_string]
	'''
	parags = lesson_str.split(sep_bar2)
	...
	for p in parags:
		p_end = p_start + len(p)
		sents = p.split(sep_bar1)
		
		s_start = p_start
		st_sents = []
		for s in sents:
			s_end = s_start + len(s)
			st_sents.append([s_start, s_end, s])
			s_start = s_end + len(sep_bar1)
		
		st_parags.append([p_start, p_end, st_sents])
		p_start = p_end + len(sep_bar2)
	return st_parags
